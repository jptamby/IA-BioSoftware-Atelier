...
def calculate_burger_price(ingredients_list):
    ...
    base_price = sum_ingredients_recursive(ingredients_copy)
...
def getMeat():
    ...
    try:
        # Éviter eval() qui est dangereux - utiliser directement la valeur
        meat = meat_type
        time.sleep(0.1)
    except Exception:
        meat = "Mystery Meat"
...
def SaveBurger(burger):
    # Corriger le problème de fichier non fermé
...
def MAIN():
    ...
    try:
        ...
        if burger:  # Vérifier que burger n'est pas None
            SaveBurger(burger)
    except Exception as e:
        print(f"An error occurred: {e}")

# CORRECTION PRINCIPALE : __name__ au lieu de **name**
